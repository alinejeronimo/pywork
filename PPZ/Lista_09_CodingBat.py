...
def maior_ponta(nums):
  if nums[0] > nums[-1]:
    b = [nums[0], nums[0], nums[0]]
    return b
  else:
    b = [nums[-1], nums[-1], nums[-1]]
    return b
...
def squirrel_play(temp, is_summer):
  if 90 >= temp >= 60 and not is_summer:
    return True
  if 100 >= temp >= 60 and is_summer:
    return True
  else:
    return False
...
def pego_correndo(speed, is_birthday):
  if speed <= 60:
    return 0
  elif 65 >= speed > 60 and is_birthday:
    return 0
  elif 81 >= speed > 60 and not is_birthday:
    return 1
